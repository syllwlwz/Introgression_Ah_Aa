...
MZha <- referencecn.mops(cases=Miashalleri,controls=Zapahalleri, minWidth = 2, minReadCount=6)
resCNMOPSMZha <- calcIntegerCopyNumbers(MZha)

ZMha <- referencecn.mops(cases=Zapahalleri,controls=Miashalleri, minWidth = 2, minReadCount=6)
...
require(openxlsx)
write.xlsx(frequ3MZha,"CnvregionswithmeanhalleriMiasZapahalleri_minRead6_WL500_minL2_min5samples.xlsx",col.names=TRUE,row.names=FALSE,overwrite=T)

frequbigMZha<-read.xlsx("CnvregionswithmeanhalleriMiasZapahalleri_minRead6_WL500_minL2_min5samples.xlsx",1)
...
write.xlsx(frequbigMZha_lyrGenes_df,"MZha_temp.xlsx",col.names=TRUE,row.names=FALSE)
...
#119 unique genes
write.xlsx(Thal_MapMan_frequbigMZha_lyrGenes_df_desc_w_orthogroup_highsupport,"GeneshalleriMiasZapahallericnvregions_minRead6_WL500_minL2_minoverlap90percentofgene_atleast90percentsupport_atleast5samples.xlsx",sheetName="GeneshalleriWulmoutgroupcnmops",col.names=TRUE,row.names=FALSE,overwrite=T)
...
require(openxlsx)
write.xlsx(frequ3ZMha,"CnvregionswithmeanhalleriZapaMiashalleri_minRead6_WL500_minL2_min5samples.xlsx",col.names=TRUE,row.names=FALSE,overwrite=T)
frequbigZMha<-read.xlsx("CnvregionswithmeanhalleriZapaMiashalleri_minRead6_WL500_minL2_min5samples.xlsx",1)
...
write.xlsx(frequbigZMha_lyrGenes_df,"ZMha_temp.xlsx",col.names=TRUE,row.names=FALSE)
...
#133 unique genes
write.xlsx(Thal_MapMan_frequbigZMha_lyrGenes_df_desc_w_orthogroup_highsupport,"GeneshalleriZapaMiashallericnvregions_minRead6_WL500_minL2_minoverlap90percentofgene_atleast90percentsupport_atleast5samples.xlsx",sheetName="GeneshalleriWulmoutgroupcnmops",col.names=TRUE,row.names=FALSE,overwrite=T)
...
write.table(MZhaonly,"MZha_cnvs_only_strict.table",row.names=F,sep="\t")
write.table(ZMhaonly,"ZMha_cnvs_only_strict.table",row.names=F,sep="\t")
