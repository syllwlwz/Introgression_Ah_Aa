...
PZha <- referencecn.mops(cases=Piekhalleri,controls=Zapahalleri, minWidth = 2, minReadCount=6)
resCNMOPSPZha <- calcIntegerCopyNumbers(PZha)

ZPha <- referencecn.mops(cases=Zapahalleri,controls=Piekhalleri, minWidth = 2, minReadCount=6)
...
require(openxlsx)
write.xlsx(frequ3PZha,"CnvregionswithmeanhalleriPiekZapahalleri_minRead6_WL500_minL2_min5samples.xlsx",col.names=TRUE,row.names=FALSE,overwrite=T)

frequbigPZha<-read.xlsx("CnvregionswithmeanhalleriPiekZapahalleri_minRead6_WL500_minL2_min5samples.xlsx",1)
...
write.xlsx(frequbigPZha_lyrGenes_df,"PZha_temp.xlsx",col.names=TRUE,row.names=FALSE)
...
#119 unique genes
write.xlsx(Thal_MapMan_frequbigPZha_lyrGenes_df_desc_w_orthogroup_highsupport,"GeneshalleriPiekZapahallericnvregions_minRead6_WL500_minL2_minoverlap90percentofgene_atleast90percentsupport_atleast5samples.xlsx",sheetName="GeneshalleriWulmoutgroupcnmops",col.names=TRUE,row.names=FALSE,overwrite=T)
...
require(openxlsx)
write.xlsx(frequ3ZPha,"CnvregionswithmeanhalleriZapaPiekhalleri_minRead6_WL500_minL2_min5samples.xlsx",col.names=TRUE,row.names=FALSE,overwrite=T)
frequbigZPha<-read.xlsx("CnvregionswithmeanhalleriZapaPiekhalleri_minRead6_WL500_minL2_min5samples.xlsx",1)
...
write.xlsx(frequbigZPha_lyrGenes_df,"ZPha_temp.xlsx",col.names=TRUE,row.names=FALSE)
...
#133 unique genes
write.xlsx(Thal_MapMan_frequbigZPha_lyrGenes_df_desc_w_orthogroup_highsupport,"GeneshalleriZapaPiekhallericnvregions_minRead6_WL500_minL2_minoverlap90percentofgene_atleast90percentsupport_atleast5samples.xlsx",sheetName="GeneshalleriWulmoutgroupcnmops",col.names=TRUE,row.names=FALSE,overwrite=T)
...
write.table(PZhaonly,"PZha_cnvs_only_strict.table",row.names=F,sep="\t")
write.table(ZPhaonly,"ZPha_cnvs_only_strict.table",row.names=F,sep="\t")
